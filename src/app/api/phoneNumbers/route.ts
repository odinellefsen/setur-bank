...
export async function POST(req:any) {
  ...
  try {
    ...
    const { phoneNumberID, pTal, phoneNumber } = await req.json();
    
    connection = await pool.getConnection();
    console.log("connected to the database");

    // Prepare your SQL statement
    const sql = `INSERT INTO phoneNumbers (phoneNumberID, pTal, phoneNumber) VALUES (?, ?, ?)`;

    // Execute the query
    await connection.query(sql, [phoneNumberID, pTal, phoneNumber]);
